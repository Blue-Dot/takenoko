...
class Panda(Objective):
    ...
    # This is to add onto the plain surface that is already there (ie add the bamboo)
    def update_surface(self):
        self.create_surface()

        for index, i in enumerate(self.bamboo):
            bamboo_image = pygame.image.load(
                c.image_bamboo[c.tile_colours.index(i)])
            bamboo_image = pygame.transform.scale(
                bamboo_image, c.objective_bamboo_dimensions)
            self.surface.blit(bamboo_image, ((
                c.objective_width - c.objective_bamboo_dimensions[0]) / 2, index * 30 + 50))
...
class Gardener(Objective):
    ...
    def valid(self) -> bool:
        ...
        # These are the tiles that have allready been checked (ie if the objective has three bamboo, you must have three different tiles, and not just check one tile three times)
        exclude = []
...
class Plots(Objective):
    ...
    # not actually update_surface by the way... it is only used in the creation of the surface, but too late to change it :b
    def update_surface(self):
        self.create_surface()
